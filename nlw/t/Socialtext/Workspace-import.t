...
for my $overwrite ( 0 .. 1 ) {
    ...
    # The actual tests start here ...
    {
        ...
        ok(
            $admin->has_user($user),
            'devnull1@socialtext.com is in admin workspace'
        );

        is(
            $user->first_name(), $singapore,
            'user first name is Singapore (in Chinese)'
        );
        is(
            $user->last_name(), $dot_net,
            'user last name is dot net (umlauts on o)'
        );
        ok(
            $user->password_is_correct('something or other'),
            'password survived import'
        );

        ok(
            Socialtext::EmailAlias::find_alias('admin'),
            'email alias exists for admin workspace'
        );

        ok(
            $admin->role_has_permission(
        ...
        like(
            $page->content(), qr/new front page content/,
        ...
        like(
            $page->content(), qr/organize information/,
        ...
        like(
            $@, qr/cannot restore/i,
            'cannot restore over an existing workspace'
        );
